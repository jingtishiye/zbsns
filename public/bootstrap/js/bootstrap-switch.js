...
(function (global, factory) {
  ...
  function getClasses(options, id) {
    ...
    return [state ? 'on' : 'off', size, disabled ? 'disabled' : undefined, readonly ? 'readonly' : undefined, indeterminate ? 'indeterminate' : undefined, inverse ? 'inverse' : undefined, id ? 'id-' + id : undefined].filter(function (v, index, array) {
  ...
  function prvgetElementOptions() {
    return {
      ...
      status: this.$element.data('status'),
  ...
  function prvcontainerPosition() {
    ...
    var state = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : _this2.options.state;
