...
function send_message($uid,$subject,$mail_subject,$type){
global $conf;
    $userinfo = user_qx_cache($uid);
    $user = user_read_cache($uid);
    if(!empty($userinfo)){
      if($userinfo['extend']['notify'][$type]==1){
                        send_sys_message($uid,$subject);
      }
      if($userinfo['extend']['notify']['mail_'.$type]==1&&$userinfo['extend']['mailstatus']==1){
          send_mail($conf['sitename'], $user['usermail'], $mail_subject,$mail_subject);
      }	
    }
...
/*
	...
	/thread/create/1
*/
function module_select($type){
global $conf;
if($type==1&&!in_array('topics',$conf['module_arr'])){
$type=false;
}
if($type==2&&!in_array('docs',$conf['module_arr'])){
$type=false;    
}
...
function url($module='index',$controller='',$action='', $extra = array()) {
	// 用于生成最原始的
...
//检测是否关注和收藏
function hasfocus($id,$type){
	global $uid;
	$r = db_find_one('usersandother',array('uid'=>$uid,'did'=>$id,'type'=>$type));
	if($r){
       return $r;
	}else{
       return false;
	...
	if($ajax) {
		...
		if(IN_CMD) {
			...
			if(defined('MESSAGE_HTM_PATH')) {
				...
				include $conf['view_path']."message.htm";
			}
		}
	}
...
include_once BASEPHP_FUNPATH.'xn_html_safe.func.php';
