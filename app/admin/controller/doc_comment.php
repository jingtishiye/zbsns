...
if (empty($action) || $action == 'list') {
    ...
    $pagination = pagination(r_url('doc_comment-list', array('page' => 'pagenum')), $totalnum, $page, $pagenum);
    include ADMIN_PATH . "view/comment_list.html";

} else if ($action == 'delete') {
    $id     = param('id');
    $result = db_update('comment',array('id'=>$id), ['status' => -1]);

    db_update('comment',array('pid'=>$id), ['status' => -1]);
