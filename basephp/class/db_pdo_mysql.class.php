...
class db_pdo_mysql {
	...
	public function sql_find_column($sql, $key = NULL) {
	...
	public function find_column($table, $cond = array(), $key = '') {
		
		$cond = db_cond_to_sqladd($cond);

		
		
		return $this->sql_find_column("SELECT $key FROM {$this->tablepre}$table $cond",$key);
		
	}
	public function find_all($table, $cond = array(), $orderby = array(), $key = '', $col = array()) {
...
$cols = implode(',', $col);
		...
		return $this->sql_find("SELECT $cols FROM {$this->tablepre}$table $cond$orderby", $key);
	...
	public function sum($table, $field, $cond = array()) {
		$sqladd = db_cond_to_sqladd($cond);
		$sql = "SELECT SUM($field) AS sumval FROM `$table` $sqladd";
		$arr = $this->sql_find_one($sql);
		return !empty($arr) ? intval($arr['sumval']) : $arr;
	}
	...
	public function minid($table, $field, $cond = array()) {
		$sqladd = db_cond_to_sqladd($cond);
		$sql = "SELECT MIN($field) AS minid FROM `$table` $sqladd";
		$arr = $this->sql_find_one($sql);
		return !empty($arr) ? intval($arr['minid']) : $arr;
	}
